...
string get_word(stringstream& sin){
    string s;
    sin >> s;
    if (s[0]=='"'){
        s.erase(s.begin());
...
value* get_value(stringstream& sin){
    string s;
    sin >> s;
    for (auto& i:_exec_value)
        if (i.first==s){
...
value* get_value_to_list(stringstream& sin){
    string s;
    sin >> s;
    if (s=="true" || s=="True") return new value(true);
    if (s=="false" || s=="False") return new value(false);
...
string get_value(stringstream& sin,string str,value** v){
    stringstream ss;
    string s,s_ret;
    ss << str;
    ss >> s;
...
string get_value_to_list(stringstream& sin,string str,value** v){
    stringstream ss;
    string s,s_ret;
    ss << str;
    ss >> s;
...
value* get_value(stringstream& sin,string s){
    ...
    if (s[0]=='['){
        ...
        while (1){
            list* tmp = new list();
            s = get_value_to_list(sin,s,&tmp->key);
            cur->next = tmp;
            cur = cur->next;
            if (cur->key->check_string() && cur->key->key_string == "End of list") break;
        }
